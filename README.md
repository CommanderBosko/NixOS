...
- **Declarative Canon TS9500 printer queue** (`modules/printing.nix`, part of `desktopModules`): `Canon_TS9500_series` declared via `hardware.printers.ensurePrinters` with a checked-in driverless PPD (`modules/printing/canon-ts9500.ppd`) and a `dnssd://` URI, set as the default; `cups-browsed` is disabled entirely rather than relied on for runtime discovery, closing a recurring "queue doesn't survive reboot" race for good
...
**2026-09-23 (session 116, latest)** — Root-caused and permanently fixed a live printer failure: `cups-browsed`'s auto-created queue hadn't survived a reboot since 2026-09-21 (a discovery-timing race), and a hand-re-added queue picked the wrong driver (Gutenprint's "Apollo P-2100" instead of the TS9500's), silently discarding every job while CUPS reported it "completed." Replaced `cups-browsed` outright with a static, declarative `Canon_TS9500_series` queue (checked-in driverless PPD) rather than patching the race a third time (`8085a0e`); `printer-diagnose` rewritten for the new setup plus a real `pipefail`/`timeout` bug fix (`1c8c36e`). Live on gaming (rebuilt); natalie-laptop can print via a live-applied queue but still needs its config pulled+switched; laptop hasn't been touched and can't print yet. Also: `xwayland-satellite` pin re-checked, still holds (no change).

**2026-09-22 (session 115)** — No NixOS config changes; two `manager`-agent-overseen runs. `/dream` mined 37 transcripts (9 memory updates + 2 new files, no repo changes) and flagged a real governance contradiction rather than auto-resolving it: does a `manager` agent's task-level pre-delegated merge authority satisfy the ask-first gate? Resolved by direct user decision — yes, going forward. `/improve-system` opened PR #26 (16 verified skill-doc/script fixes + 7 permission entries), closing the `review-improve-system-pr` `references/` guardrail gap open since PR #25; CI went green and it was merged (`8716fa2`). Gaming still needs a rebuild+reboot to pick up the global skill fixes.

**2026-09-21 (session 114)** — A repo-wide reorganization: config moved next to the modules that own it (dev toolchain, Claude packages, flatpak, SDDM, and bootloader/fonts/tailscale into `desktopModules`), the shared DMS stack and niri Home Manager config extracted, vpn-server split into focused files, and `bosko-claude.nix` now auto-wires Claude skills and agents from the directory listing (a new skill needs only `git add`). CI's deep-eval loop became one shared script that reports every failing host, a new `lib.moduleSmoke` evaluates modules no host imports, and the README and CLAUDE.md module lists were cut down to point at `flake.nix`. Verified by a 4-host deep-eval, all DE smoke checks and a before/after diff of evaluated contents. The Claude tooling secrets (`tailscale-mcp-env`, `discord-webhook-url`) moved to a new desktop-only `secrets/desktop.yaml` that vpn-server can't decrypt, and both credentials were rotated; along the way a secret-exposure incident (the `add-secret` skill wrongly claimed `!` commands aren't logged) was contained and the skill fixed to capture values in a separate terminal. Weekly `improve-system` PR #25 was reviewed and merged. Gaming is rebuilt; laptop and natalie-laptop still need a rebuild.

## Roadmap

- Rebuild + reboot laptop and natalie-laptop (gaming is current as of 2026-09-23, gen 422, but also owes a small rebuild for PR #26's global skill fixes): activate managed Claude Code policy, FinanceGuru, package consolidation changes, the OnlyOffice font fix, the static Canon printer queue, and natalie-laptop's Plasma removal (niri-only now); remove hand-maintained `~/.ssh/config` on each host afterward
