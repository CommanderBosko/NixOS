...
**2026-09-20 (session 113, latest)** — Routine maintenance, no NixOS config changes beyond the lock. `/flake-update-verify` bumped `disko`, `dms`, `financeguru`, `home-manager`, `nixpkgs`, and `sops-nix` (`f978b9c`); `nix flake check` and a per-host deep-eval passed on all four hosts, and the bump is committed but **not yet applied** to any host (it stacks on the still-unapplied 2026-09-07 bump, so one `/fleet-rollout` covers both). On the pi-hole host (outside this repo), the `pihole-updatelists` config was found missing HaGeZi TIF and still listing an RPiList malware feed that had fallen out of the live list set; both are now in sync and gravity was rebuilt (29 lists). The `xwayland-satellite` 0.8.1 pin was re-checked twice against upstream issue #156 — still open, no newer release than 0.8.2 — so it stays. Close-out also corrected three stale `project-state.md` items that still described that pin as untested.
...
**2026-09-16 (session 111)** — Pure Claude-ecosystem maintenance, no NixOS config/host changes. `skill-upgrade` audited all 70 skills for embedded creative examples (a fully-composed sentence offered as a copy-target, vs. a fixed schema — fine); one real hit, `session-closer`'s WireGuard/sops-nix worked example, rewritten as an interface instead (`7b8fd41`). Split 16 new `references/*.md` files out of both CLAUDE.md files and 14 SKILL.md files, replacing heavy rare-case blocks (Gotchas, procedures) with one-line pointers — two of 5 parallel review forks got contaminated by the orchestrator's own status narration mid-run and were stopped/redone directly; caught and fixed a real gap where 3 global skills (`research`, `agent-suggestion`, `improve-system`) were symlinked file-by-file rather than recursively, so their new `references/` dirs would've stayed invisible to `~/.claude` forever (`1c994b6`). Closed the gap for future skills: `new-skill` gained a step that routes heavy/narrow context to `references/` at draft time, and `skill-audit` gained a matching 7th rubric lens (`f284a62`). Ran `/improve-system` twice — first pass built and wired a new `skill-builder` custom sub-agent for parallel skill drafting (rewiring `skill-suggestion` to use it) and landed 11 more fixes from a 71-skill audit; second pass caught one live misfire and added 2 more Gotchas (`1f9ddc6`). All 4 commits are repo-managed global skill/agent/CLAUDE.md changes, pending a rebuild+reboot — the new agent specifically needs the reboot to be dispatchable by name.
