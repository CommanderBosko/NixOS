...
> - Run `/git-commit` to commit the updated `flake.lock` once you are satisfied.
...
- Do not auto-commit `flake.lock` — committing is the user's explicit action via `/git-commit`.
- Do not auto-rebuild or auto-dry-run — suggest it, do not invoke it.
- Do not use `git add` after the update — flake.lock will appear as a modification to an already-tracked file; staging is handled by `/git-commit`.
