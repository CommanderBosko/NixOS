...
5. **Verify the updated lock evaluates** — invoke the `flake-check` skill
   (`nix flake check`) to evaluate ALL host configs against the new inputs, then invoke
   the `deep-eval-check` skill to deep-evaluate every host's actual build graph (a
   shallow `flake-check` pass alone is not sufficient — see Gotchas).
   - Done-rule: `flake-check` completes with no evaluation errors AND `deep-eval-check`
     reports PASS for every host.
